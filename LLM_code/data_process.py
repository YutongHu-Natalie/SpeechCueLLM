    ...
    for conv_id in all_conv_id:
        ...
        for conv_turn in range(len(sentence_dict[conv_id])):
            ...
            if experiments_setting == 'few_shot':
                ...
                if audio_only == 'True':
                    temp_content_str += f'\n\nBased on the audio features, select one emotion from [{label_text_set[dataset]}].\nOutput ONLY valid JSON in this exact format:\n{{"detected_emotion_label": "<emotion>", "reason": "<brief explanation>"}}'
                elif audio_description == 'True' or audio_impression == 'True':
                    temp_content_str += f'\n\nFor {target_utterance}, based on the context and audio features, select one emotion from [{label_text_set[dataset]}].\nOutput ONLY valid JSON in this exact format:\n{{"detected_emotion_label": "<emotion>", "reason": "<brief explanation>"}}'
                else:
                    temp_content_str += f'\n\nFor {target_utterance}, based on the context, select one emotion from [{label_text_set[dataset]}].\nOutput ONLY valid JSON in this exact format:\n{{"detected_emotion_label": "<emotion>", "reason": "<brief explanation>"}}'
