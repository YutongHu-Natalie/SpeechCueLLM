"""
Qwen API inference script for emotion recognition in conversations.
Uses OpenAI-compatible API to call Qwen3.5-35B-A3B on a remote server.
...
def extract_emotion_from_json(output, label_set):
    ...
    Handles Qwen's thinking output which may wrap JSON in <think>...</think> blocks.
    """
    if not output or output.strip() == '':
        return None

    # Strip thinking block if present (Qwen3.5 thinking mode)
...
def parse_input_for_qwen(input_text, dataset):
    """
    Parse the processed input text to extract conversation_history, target_utterance, and audio_features.
    Identical to the OpenAI parser since the input format is the same.
    """
...
def create_qwen_messages_zero_shot(conversation_history, target_utterance, audio_features, label_str):
    """
    Create messages for Qwen API call using zero-shot prompt format.
    Uses 'system' role (Qwen does not use 'developer' role).
    """
    system_message = f"""Now you are expert of emotional analysis for dialogues. Please select one emotion label word from [{label_str}] and always respond in strict json format."""
...
def create_qwen_messages_few_shot(conversation_history, target_utterance, audio_features, label_str):
    """
    Create messages for Qwen API call using few-shot prompt format with examples.
    Uses 'system' role (Qwen does not use 'developer' role).
    """
    system_message = f"""Now you are expert of emotional analysis for dialogues. Please select one emotion label word from [{label_str}] and always respond in strict json format."""
...
def run_qwen_inference(client, model_name, messages, enable_thinking=False, max_retries=3):
    """
    Run inference using Qwen via OpenAI-compatible API with retry logic.
    """
    extra_body = {
        "top_k": 20,
    ...
    for attempt in range(max_retries):
        try:
            response = client.chat.completions.create(
                model=model_name,
                messages=messages,
                max_tokens=81920 if enable_thinking else 4096,
                temperature=1.0,
                top_p=0.95,
...
def main():
    parser = argparse.ArgumentParser(description='Qwen API inference for emotion recognition')
    ...
    parser.add_argument('--base_url', type=str, default=None,
                        help='Base URL for the remote Qwen server (e.g. http://host:8000/v1). '
                             'Falls back to QWEN_BASE_URL env variable.')
    parser.add_argument('--api_key', type=str, default=None,
                        help='API key for the remote server. Falls back to QWEN_API_KEY env variable. '
                             'Use "EMPTY" if the server requires no authentication.')
    ...
    parser.add_argument('--enable_thinking', action='store_true',
                        help='Enable Qwen thinking mode (chain-of-thought before answer). '
    ...
    base_url = args.base_url or os.environ.get('QWEN_BASE_URL')
    if not base_url:
        raise ValueError(
            "Qwen server base URL must be provided via --base_url or QWEN_BASE_URL environment variable"
        )

    api_key = args.api_key or os.environ.get('QWEN_API_KEY', 'EMPTY')

    # Initialize OpenAI-compatible client pointing at the remote Qwen server
    ...
    if args.experiments_setting == 'few_shot':
        create_messages_fn = create_qwen_messages_few_shot
        print("Using few-shot prompting with examples")
    else:
        create_messages_fn = create_qwen_messages_zero_shot
        print("Using zero-shot prompting")

    thinking_label = "enabled" if args.enable_thinking else "disabled"
    ...
    for idx, sample in enumerate(tqdm(test_data, desc="Processing")):
        ...
        # Parse input
        conversation_history, target_utterance, audio_features = parse_input_for_qwen(input_text, args.dataset)

        # Create messages
        messages = create_messages_fn(conversation_history, target_utterance, audio_features, emotional_label_str)

        # Run inference
        output = run_qwen_inference(client, args.model_name, messages, enable_thinking=args.enable_thinking)
        ...
        if (idx + 1) % 50 == 0 or idx == 0:
            ...
            print(f"LLM Output: {output[:300]}{'...' if len(output) > 300 else ''}")
            print(f"{'='*80}\n")
